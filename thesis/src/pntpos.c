...
*			int      *ns       O   参与定位的卫星的个数
* return : status(1:ok, 0 : error)
* 注：rescode函数完成了解算模型的线性化、以及伪距补偿、加权参数估计等工作，是最小二乘计算的基础，
*     可以这么说，最小二乘中用到的所有参数（除卫星位置外）都在这个函数里计算。
*----------------------------------------------------------------------------*/
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        /* geometric distance/azimuth/elevation angle
		   计算卫星和当前接收机位置之间的几何距离和receiver-to-satellite方向的
		   单位向量，然后检验几何距离是否大于0。
		   satazel，计算在接收机位置处，站心坐标系中卫星的方向角和仰角，检验仰角是否大于等于截断值。
		*/
		...
		*/
		...
		/*
		将参与定位的卫星定位有效性标志设为1，给当前卫星的伪距残余赋值，参与定位的卫星个数ns+1。
		*/
        vsat[i]=1; resp[i]=v[nv]; (*ns)++;
        
        /* error variance
		通过varerr函数，计算此时的导航系统误差，然后累加计算用户测距误差（URE）。
		*/
                  ...
                  char *msg)
{
	...
	残差检测用的是卡方检验，其实这个检验对低精度板卡来说有点严格，
	如果解算对象是低精度板卡，这个检测应该静止掉，否则有很多结果是
	通过不了的。
	*/
	...
	/* dop值计算，该函数可以看出，dop值仅与卫星的几何分布有关系，
	   azels为卫星们的方向角仰角。
	*/
	...
	*/
    for (i=0;i<MAXITR;i++) {
		...
		*/
		...
		/*
		泰勒级数线性化后的迭代计算后的残差范数小于1e-4时结束迭代，
		一般6次左右就能结束迭代了
		*/
        if (norm(dx,NX)<1E-4) {
            sol->type=0;
                  ...
                  char *msg)
{
    ...
    /* raim fde
	接收机自主完好性监测和故障检测排除，即通过每次排除一颗卫星进行解算，然后
	选取残差最小的一次解算结果作为最终解算结果。
	*/
    if (!stat&&n>=6&&opt->posopt[4]) {
        stat=raim_fde(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,msg);
    }
    /* estimate receiver velocity with doppler
	   基于多普勒频移观测值完成速度计算，多普勒频移观测值（obsd_t中的D）如果没有则
	   解算失败，如果仍要获取速度，只能通过位置进行估计
	*/
